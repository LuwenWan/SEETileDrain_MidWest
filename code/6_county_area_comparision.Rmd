...
setwd("C:/wanluwen/Code/SEETileDrain_MidWest/Midwest_Diss_20230802")
tileDir <- ":/wanluwen/Code/SEETileDrain_MidWest/Midwest_Diss_20230802/"
...
names(my_county)
sum(my_county$tile_m2, na.rm = TRUE)
...
data <- read_csv(paste0(tileDir,"Midwest_class_area_by_county_notnull_202406.csv"),show_col_types = FALSE)
names(data)
...
# area 
sum(data_classified$classified_Tile)
...
sum(data_classified_final$classified_Tile_m2)
```
...
merge_NA_classified <- merge %>%
  filter(is.na(classified_Tile_m2))  # 210
...
fname <- paste0(tileDir, 'Fig6_County_level_accuracy_SpatialResiduals.pdf'); fname
ggsave(filename = fname,
