...
dataIndir <- "C:/wanluwen/Code/SEETileDrain_MidWest/Midwest_Diss_20230802/"
...
load(paste0(dataIndir, "mydata.RData")) 
...
td_levels <- c("Climate","Landscape characteristics","Surface reflectance","LST","SMAP") 
td_labels <- c("Climate","Landscape characteristics","Surface reflectance","LST","SMAP") 
td_colors <- c("#1f78b4","#fb9a99","#33a02c","#a6d854","#a6cee3") 
gear.cols <- c("Climate" = "#1f78b4",
               "Landscape characteristics" = "#fb9a99",
               "Surface reflectance" = "#33a02c",
               "LST" = "#a6d854",
               "SMAP" = "#a6cee3") 
...
nrow(nontile_tain) + nrow(tile_tain) + nrow(nontile_test) + nrow(tile_test) 

# these data was collected based on different train and test 
tile_train2 <- read_csv(paste0(dataIndir,"Midwest_tile_train_sampleRe_scale_allVars_30_noSAR_20230802.csv"),show_col_types = FALSE)
nontile_train2 <- read_csv(paste0(dataIndir,"Midwest_nontile_train_sampleRe_scale_allVars_30_noSAR_20230802.csv"),show_col_types = FALSE)
test2 <- read_csv(paste0(dataIndir,"/Midwest_test_sampleRe_scale_allVars_30_noSAR_20230802.csv"),show_col_types = FALSE)
nrow(tile_train2) + nrow(nontile_train2) + nrow(test2) # 60938
nrow(tile_train2) + nrow(nontile_train2) # 48982
nrow(test2) # 11956
# 64108 - 60938 = 3170, so we missed 3170/64108=5% points, some holes in input variables
...
nrow(test_df_nontile)

nrow(train_df_tile) + nrow(test_df_tile)
nrow(train_df_nontile) + nrow(test_df_nontile)
...
# order in Table 2
finallist <- c("NDVI_Grow_max", "NDWI_Spring_max","NDWI_Summer_max","Tr_SWIR1_Grow_max","Tr_SWIR2_Spring_max","Tr_SWIR2_Summer_max",
"aet_grow_terra","pr_grow","aridity_spr","aridity_preGrow_3yr",
"dayLST_median_spr", "dayLST_median_grow","dayLST_range_spr","dayLST_median_summ","dayLST_range_grow","dayLST_range_summ",
 "diffLST_median_grow","diffLST_median_spr",
"nightLST_max_summ", "nightLST_max_spr",
 "SMP_median_summ","SUSM_median_spr","SUSM_range_spr",
 "clay_mean_0_5","ksat_mean_0_5",  "paw_mean_0_5",
"slope_mean","soilDrainClass","CanalDitchD", "HLR", "cropland")
...
df1 <- df_select %>%
  dplyr::mutate(
    group = ifelse(str_detect(string = Bands, pattern = 'NDVI.|NDWI.|Tr_swir.'), 'Surface reflectance', ''),
    group = ifelse(str_detect(string = Bands, pattern = 'DayLST_|DiffLST_|NightLST_'), 'LST', group),
    group = ifelse(str_detect(string = Bands, pattern = 'SMP.|SSM.|SUSM.'), 'SMAP', group),
    group = ifelse(str_detect(string = Bands, pattern = 'AET.|Aridity.|Precip_'), 'Climate', group),
    group = ifelse(str_detect(string = Bands, pattern = 'Soil_drain_class|Slope_mean|HLR|Canal|Paw_|paw_|Clay_|Ksat_|Cropland'), 'Landscape characteristics', group))
...
fname <- paste0(dataIndir,today, '_','RF_R_importance_allPoints_selectVars.csv'); fname
write_csv(df1, file = fname)
```


### 3.6.1 MeanDecreaseAccuracy
```{r }
# 20240429_RF_R_importance_allPoints_selectVars.csv
df1 <- read_csv(paste0(datadir,"20240728", '_','RF_R_importance_allPoints_selectVars.csv'),show_col_types = FALSE)
unique(df1$group)
...
accuDecrease <- df1 %>%
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, linewidth=1.5),  # panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        ...
        legend.position = c(0.6, 0.07),legend.title = element_blank(),
...
MDA <- read_csv(paste0(datadir,"20240728_RF_R_importance_allPoints_selectVars.csv"),show_col_types = FALSE)
...
phi_df <- read_csv(paste0(dataIndir,'phi_df_1percent.csv'),show_col_types = FALSE)
...
shapleyPlot <- phi_df_mean_group %>%
  ...
  theme(panel.border = element_rect(colour = "black", fill=NA, linewidth=1.5),  # panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        ...
        # plot.title = element_text(hjust = 0.5,color = "black", size = title_size),
        legend.position = c(0.6, 0.1),legend.title = element_blank(),
        legend.text = element_text(color = "black", size = font_size),
        axis.title = element_blank(),
        axis.text = element_text(color = "black", size = font_size),
        legend.background = element_rect(color = "black",linewidth = 0.5)) +
   annotate("text", label = "(a)", x = 0.9, y = 0.004, hjust = 0, vjust = 0, size = 4)
...
impor <- read_csv(paste0(dataIndir,'20240728_RF_R_importance_allPoints_selectVars.csv'),show_col_types = FALSE)
names(impor)
...
ScorePlot <- shapley_MDA_Gini %>%
  ...
  annotate("text", label = "(b)", x = 0.9, y = 10, hjust = 0, vjust = 0, size = 4)
...
load("S:/Users/luwen/Tile_Mapping_States/1_MyTile/Midwest_Diss_20230802/mydata.RData")
```
