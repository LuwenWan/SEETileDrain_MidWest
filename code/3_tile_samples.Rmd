...
### Set work dir ----------------------------------------------------------
dataout <- "C:/wanluwen/Code/SEETileDrain_MidWest/figure"
...
# get state boundary coordinates for HydroShare
```{r}
current_crs <- st_crs(gom)
print(current_crs)

# Transform to EPSG:4326 if not already in decimal degrees
if (current_crs != 4326) {
    gom_4326 <- st_transform(gom, 4326)
}

# Get the bounding box in decimal degrees
bounding_box <- st_bbox(gom_4326)
print(bounding_box)
```
...
```{r}
tile_tain <- st_read(paste0(ground_dir,'20240615_train_tile_nontile.shp'))
tile_test <- st_read(paste0(ground_dir,'20240615_test_tile_nontile.shp'))
names(tile_test)
